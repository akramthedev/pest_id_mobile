...
const formatDateForCreatedAt = (date) => {
  ...
  useEffect(()=>{
    const x = async()=>{
      try{
        ...
        const response = await axiosInstance.get(`${ENDPOINT_API}getFarmsWithGreenhouses/${parseInt(userIdNum)}`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
  ...
  const createCalculation = async () => {
      ...
      let formData = {
        ...
        serre_id : selectedGreenhouse !== null && selectedGreenhouse !== '' ? selectedGreenhouse : null,
        farm_id : selectedFarm !== null && selectedFarm !== "Aucune ferme disponible" && selectedFarm !== '' ? selectedFarm : null
      };
